...
const EXEMPT: Record<string, string> = {
  ...
  'telegram-subagent-reply-fix.py':
    'scheduled/CLI repair tool (telegram-subagent-hook-check task, SUBAGENTHOOKCHECK926): it rewrites a sub-agent\'s settings.json hook entries into the defensive absolute-path form and reports which agents to restart; it is invoked by the schedule, not registered as a hook itself',
